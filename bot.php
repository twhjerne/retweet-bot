...
foreach ($tweets->statuses as $tweet) {
	...
	$existing_tweet = mysqli_query('SELECT * FROM retweets WHERE id = "' . $tweet->id . '"');
	...
	if (!mysqli_num_rows($existing_tweet) && substr($tweet->text, 0, 2) != 'RT' && substr($tweet->text, 0, 1) != '@') {
		...
		$result = mysqli_query("INSERT INTO retweets (id, text, screen_name) VALUES ('" . $tweet->id . "','" . utf8_decode($tweet->text) . "','" . utf8_decode($tweet->user->screen_name) . "')");
